    ...
    public static class OpenWavParser
    {
        ...
        public static bool IsWAVFile(byte[] wavFile)
        {
            if (wavFile.Length > 12)
            {
                ...
                Buffer.BlockCopy(wavFile, 0, data, 0, data.Length);
                string _chunkID = ByteArrayToString(data);
                Buffer.BlockCopy(wavFile, 8, data, 0, data.Length);
        ...
        public static AudioClip ByteArrayToAudioClip(byte[] wavFile, string name = "", bool stream = false)
        {
            ...
            if (IsWAVFile(wavFile))
            {
                ...
                int _subchunk1Size = BitConverter.ToInt32(wavFile, 16);
                int _audioFormat = BitConverter.ToInt16(wavFile, 20);
                int _numChannels = BitConverter.ToInt16(wavFile, 22);
                int _sampleRate = BitConverter.ToInt32(wavFile, 24);
                ...
                Resolution _bitsPerSample = (Resolution)BitConverter.ToInt16(wavFile, 34);
                // PCM WAV method:
                if (_audioFormat == 1)
                {
                    ...
                    for (int i = _dataIndex; i < wavFile.Length; i++)
                        if (wavFile[i] == 'd' && wavFile[i + 1] == 'a' && wavFile[i + 2] == 't' &&
                            wavFile[i + 3] == 'a')
                        {
                    ...
                    // Data parameters:
                    int _subchunk2Size = BitConverter.ToInt32(wavFile, _dataIndex); // Data size (Subchunk2Size).
                    ...
                    AudioClip audioClip = AudioClip.Create(name, _sampleCount / _numChannels, _numChannels, _sampleRate,
                        stream);
        ...
        public static byte[] AudioClipToByteArray(AudioClip clip, Resolution res = Resolution._16bit)
        {
            ...
            var wavFile = new List<byte>();
            // RIFF header:
            int size = (int)res / 8;
            wavFile.AddRange(new[] { (byte)'R', (byte)'I', (byte)'F', (byte)'F' }); // "RIFF"
            wavFile.AddRange(BitConverter.GetBytes(samples.Length * size + 44 - 8)); // ChunkSize
            wavFile.AddRange(new[] { (byte)'W', (byte)'A', (byte)'V', (byte)'E' }); // "WAVE"
            wavFile.AddRange(new[] { (byte)'f', (byte)'m', (byte)'t', (byte)' ' }); // "fmt "
            wavFile.AddRange(BitConverter.GetBytes(16)); // Subchunk1Size (the next 16 bytes)
            wavFile.AddRange(BitConverter.GetBytes((ushort)1)); // AudioFormat (1 for PCM)
            wavFile.AddRange(BitConverter.GetBytes((ushort)clip.channels)); // NumChannels
            wavFile.AddRange(BitConverter.GetBytes(clip.frequency)); // SampleRate
            wavFile.AddRange(BitConverter.GetBytes(clip.frequency * clip.channels * size)); // ByteRate
            wavFile.AddRange(BitConverter.GetBytes((ushort)(clip.channels * size))); // BlockAlign
            wavFile.AddRange(BitConverter.GetBytes((ushort)res)); // BitsPerSample
            wavFile.AddRange(new[] { (byte)'d', (byte)'a', (byte)'t', (byte)'a' }); // "data"
            wavFile.AddRange(BitConverter.GetBytes(samples.Length * size)); // Subchunk2Size
            ...
            for (int i = 0; i < samples.Length; i++)
                switch (res)
                {
                    case Resolution._16bit:
                        ...
                        wavFile.AddRange(BitConverter.GetBytes(sample16));
                        break;
                    case Resolution._24bit:
                        int sample24 = Mathf.FloorToInt(samples[i] * 8388607);
                        byte[] data = BitConverter.GetBytes(sample24);
                        wavFile.AddRange(new[] { data[0], data[1], data[2] });
                        break;
                    case Resolution._32bit:
                        int sample32 = (int)(samples[i] * 2147483647f);
                        wavFile.AddRange(BitConverter.GetBytes(sample32));
                        break;
                }
        ...
        public static AudioClip StereoToMono(AudioClip stereoClip, bool stream = false)
        {
            ...
            AudioClip audioClip =
                AudioClip.Create(stereoClip.name, _audioBuffer.Length, 1, stereoClip.frequency, stream);
            audioClip.SetData(_audioBuffer, 0);
            return audioClip;
        }
